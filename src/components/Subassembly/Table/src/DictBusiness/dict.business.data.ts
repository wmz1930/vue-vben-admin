...
import { updateDictBusinessStatus } from '/@/api/system/base/dictBusiness';
          ...
          createConfirm({
            ...
            content: () =>
              h(
                'span',
                newStatus === 1
                  ? `确定要启用数据字典：${record.dictName}？`
                  : `确定要禁用数据字典：${record.dictName}？`,
              ),
            onOk: async () => {
              await updateDictBusinessStatus(record.id, newStatus)
                .then(() => {
                  record.dictStatus = newStatus;
                  createMessage.success(
                    newStatus === 1
                      ? `已成功启用数据字典：${record.dictName}`
                      : `已成功禁用数据字典：${record.dictName}`,
                  );
                })
                .catch(() => {
                  record.pendingStatus = false;
                  createMessage.error('修改数据字典状态失败');
                })
                .finally(() => {
                  record.pendingStatus = false;
                });
